...
def replace_variable(substring):
    ...
    if substring == "$":
        return "$"
    elif substring[1:] in Set.variables:
        return Set.variables[substring[1:]]
    else:
        return ""
...
def getVar(variable):
    ...
    if variable in Set.variables:
        variable = Set.variables[variable]

    elif variable.startswith("#"):
        if variable[1:] in Set.variables:
            variable = str(len(Set.variables[variable[1:]]))
        else:
            variable = "0"

    elif ":-" in variable:
        parameter, word = get_param_and_word(variable, ":-")
        if parameter in Set.variables and Set.variables[parameter]:
            variable = Set.variables[parameter]
        elif parameter not in Set.variables or (parameter in Set.variables and not Set.variables[parameter]):
            variable = word
        ...
        if parameter in Set.variables and Set.variables[parameter]:
            variable = Set.variables[parameter]
        elif parameter in Set.variables and not Set.variables[parameter]:
            variable = ""
        elif parameter not in Set.variables:
            variable = word
        ...
        parameter, word = get_param_and_word(variable, "+")
        if parameter in Set.variables or (parameter in Set.variables and not Set.variables[parameter]):
            variable = word
        elif parameter not in Set.variables:
            variable = ""
        ...
        if parameter in Set.variables and (not Set.variables[parameter].endswith(word) or not word):
            variable = Set.variables[parameter]
        if parameter in Set.variables and Set.variables[parameter].endswith(word) and Set.variables[parameter] :
            variable = Set.variables[parameter][:-len(word)]
        elif parameter not in Set.variables or (parameter in Set.variables and not Set.variables[parameter]):
            variable = ""
        ...
        variable = variable.replace("#", " ")
        parameter, word = get_param_and_word(variable, None)
        if parameter in Set.variables and (not Set.variables[parameter].startswith(word) or not word):
            variable = Set.variables[parameter]
        if parameter in Set.variables and Set.variables[parameter].startswith(word) and Set.variables[parameter]:
            variable = Set.variables[parameter][len(word):]
        elif parameter not in Set.variables or (parameter in Set.variables and not Set.variables[parameter]):
            variable = ""
